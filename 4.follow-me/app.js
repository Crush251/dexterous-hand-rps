...
async function loadDeviceConfig() {
    ...
    // let newHost = parseBaseUrl(window.location.href);
    // if (newHost) {
    //     baseHost = newHost;
...
// 将姿势数据组合为 CAN 消息的 data 字段
function combinePoseData(preset) {
    // 将 finger (6个字节) 和 palm (4个字节) 组合成 10 个字节的数据
    // 确保所有值都是数字类型（0-255 范围内的字节值）
    const fingerData = preset.finger.map(v => Number(v) & 0xFF);
    const palmData = preset.palm.map(v => Number(v) & 0xFF);
    return [...fingerData, ...palmData];
}
