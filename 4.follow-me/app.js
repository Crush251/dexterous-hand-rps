...
/** MediaPipe 标签 'Left'/'Right' ↔ 设备 side */
function mpLabelForDeviceSide(side) {
    return side === 'right' ? 'Left' : 'Right';
}
/**
 * 获取“真实”的手部标签（相对于用户而非镜像画面）
 * MediaPipe 在镜像画面中标记的 'Left' 实际是用户的 'Right'，'Right' 是用户的 'Left'
 */
function getTrueHandLabel(mediaPipeLabel) {
    // 如果 MediaPipe 说这是 'Left'，那对用户来说是 'Right'
    // 如果 MediaPipe 说这是 'Right'，那对用户来说是 'Left'
    return mediaPipeLabel === 'Left' ? 'Right' : 'Left';
}
function findLandmarksForSide(multiHandLandmarks, multiHandedness, deviceSide) {
    const want = mpLabelForDeviceSide(deviceSide);
    for (let i = 0; i < multiHandLandmarks.length; i++) {
        if (multiHandedness[i].label === want) {
